...
class QuestionPolicy:
    ...
    def select(
        ...
        # Get keys to skip based on exclusive groups and conditional exclusions
        excluded_keys = self._get_excluded_keys(session, attributes)
        # Also skip related attributes based on RELATED_ATTRIBUTES
        skip_keys = self.get_implied_skip_keys(session, attributes)
        all_skip_keys = excluded_keys | skip_keys
        ...
        for attr in attributes:
            if attr.id in asked_set:
                continue
            # Skip excluded and related attributes
            if attr.key in all_skip_keys:
                continue
